...
def main():
    ...
    #      SVM MODEL HERE
    # --------------------------
    logging.info("[+] Training SVM classifier (RBF kernel)...")
    model = SVC(
        kernel="rbf",
        C=3,
        gamma="scale",
...
# Use n_jobs=1 to avoid debugpy issues in VS Code
    ...
    logging.info("[+] 10-fold CV accuracy: %.4f ± %.4f", scores.mean(), scores.std())
